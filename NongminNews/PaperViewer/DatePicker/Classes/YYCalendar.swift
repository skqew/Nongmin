...
@objcMembers open class YYCalendar {
    ...
    //Jiwoon
    public init(specificCalendarLangType langType: LangType, date: String, limitDate: [String], format: String, completion: @escaping SelectHandler) {
        componentType = .limited
        limitedCalendar = LimitedCalendar.init(langType: langType, date: date, limitDate: limitDate, format: format, completion: completion)
    }
